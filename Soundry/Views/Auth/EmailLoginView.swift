...
struct EmailLoginView: View {
    ...
    @InjectedObject(\.authenticationApiViewModel) var authorizationApiViewModel:
    ...
    var canSubmit: Bool {
        return !email.trimmingCharacters(in: .whitespaces).isEmpty
            && !password.isEmpty && emailValid && passwordValid
                    ...
                    VStack(spacing: 0) {
                        ...
                        VStack(spacing: 4) {
                            PasswordInputField(
                                ...
                                onForgotPassword: {
                                    appState.navigateToForgotPassword()
                                }
                            )
                        ...
                        Button {
                            Task {
                                await authorizationApiViewModel.loginWithEmail(email: email, password: password)
                                
                                if !authorizationApiViewModel.isLoginSuccess {
